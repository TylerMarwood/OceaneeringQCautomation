...
if not (st.session_state.client_bytes and st.session_state.inform_bytes):
...
try:
    ...
    st.error(f"Could not read client file: {exc}")
    st.stop()

try:
    inform_sheets = get_sheet_names(st.session_state.inform_bytes)
except Exception as exc:
    st.error(f"Could not read INFORM file: {exc}")
    st.stop()
...
with col_cfg_client:
    ...
    # Raw preview to help user identify rows
    with st.expander("Preview raw rows (first 8 rows from row 1)", expanded=False):
        try:
            preview_df = preview_rows(st.session_state.client_bytes, client_sheet, 1, 8)
            # Highlight header and data start rows
            def _hl_client(row):
                idx = row.name  # row number (1-indexed, set in preview_rows)
                if idx == client_header_row:
                    return ["background-color:#d4edda"] * len(row)
                if idx == client_data_row:
...
with col_cfg_inform:
    ...
    with st.expander("Preview raw rows (first 8 rows from row 1)", expanded=False):
        try:
            preview_df = preview_rows(st.session_state.inform_bytes, inform_sheet, 1, 8)
...
with col_tag_client:
    client_tag_col = st.selectbox(
        ...
        st.session_state.client_headers,
        key="client_tag_col",
...
with col_tag_inform:
    inform_tag_col = st.selectbox(
        ...
        st.session_state.inform_headers,
        key="inform_tag_col",
...
if not st.session_state.comparison_ran or st.session_state.comparison_result is None:
    st.stop()
...
with tab_tags:
    ...
    with tv_col2:
        st.metric("Client-only tags", len(tv.client_only_tags),
                  delta=None if not tv.client_only_tags else f"⚠ {len(tv.client_only_tags)} not compared",
                  delta_color="off")
    with tv_col3:
        st.metric("INFORM-only tags", len(tv.inform_only_tags),
                  delta=None if not tv.inform_only_tags else f"⚠ {len(tv.inform_only_tags)} not compared",
                  delta_color="off")
    ...
    if not tv.issues:
        ...
        # Group into categories for clearer display
        dup_issues  = [i for i in tv.issues if "Duplicate" in i.reason]
        miss_issues = [i for i in tv.issues if "Duplicate" not in i.reason and "(empty)" not in i.tag]
        null_issues = [i for i in tv.issues if "(empty)" in i.tag]

        if null_issues:
            with st.expander(f"⚠️  Empty / Null Tags ({len(null_issues)} issue(s))", expanded=True):
        ...
        if dup_issues:
            with st.expander(f"🔴  Duplicate Tags ({len(dup_issues)} tag(s))", expanded=True):
                dup_df = pd.DataFrame(
                    [{"Tag": i.tag, "Sheet": i.sheet, "Reason": i.reason} for i in dup_issues]
                )
                st.dataframe(
                    dup_df.style.apply(
                        lambda _: ["background-color:#ffc7ce"] * len(dup_df.columns), axis=1
                    ),
                    use_container_width=True,
                    hide_index=True,
                )
        ...
        if miss_issues:
            ...
            if client_miss:
                with st.expander(
                    ...
                    st.dataframe(
                        cm_df.style.apply(
                            lambda _: ["background-color:#fff3cd"] * len(cm_df.columns), axis=1
                        ),
                        use_container_width=True,
                        hide_index=True,
                    )
            ...
            if inform_miss:
                with st.expander(
                    ...
                    st.dataframe(
                        im_df.style.apply(
                            lambda _: ["background-color:#fff3cd"] * len(im_df.columns), axis=1
                        ),
                        use_container_width=True,
                        hide_index=True,
                    )
        ...
        if tv.comparable_tags:
            st.info(
                f"ℹ️  {len(tv.comparable_tags):,} tag(s) are comparable and have been included "
                f"in the data comparison below.",
            )
        else:
            st.error(
...
with tab_headers:
    ...
    # Visual header grid — one chip per header
    st.markdown("#### All Headers at a Glance")

    matched_set      = set(hm.matched)
    client_only_set  = set(hm.client_only)
    inform_only_set  = set(hm.inform_only)
    ...
    with hg_col1:
        ...
        chips = "".join(_chip(h, "green") for h in hm.matched) if hm.matched else "<i>None</i>"
        st.markdown(chips, unsafe_allow_html=True)

    with hg_col2:
        st.markdown("**🟡 Client-only headers**")
        chips = "".join(_chip(h, "amber") for h in hm.client_only) if hm.client_only else "<i>None</i>"
        st.markdown(chips, unsafe_allow_html=True)

    with hg_col3:
        st.markdown("**🟡 INFORM-only headers**")
        chips = "".join(_chip(h, "amber") for h in hm.inform_only) if hm.inform_only else "<i>None</i>"
        st.markdown(chips, unsafe_allow_html=True)
...
with tab_diff:
    ...
    if df_all.empty:
        ...
        if tag_search.strip():
            view_df = view_df[
                view_df["Tag"].str.contains(tag_search.strip(), case=False, na=False)
            ]
        ...
        if view_df.empty:
            ...
            # Style: green row for match, red for mismatch
...
with tab_downloads:
    ...
    if not df_all.empty:
        ...
        st.markdown("Download the raw comparison data as a CSV file for further analysis.")
